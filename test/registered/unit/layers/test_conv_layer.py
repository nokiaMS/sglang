...
class TestConv2dLayer(unittest.TestCase):
    ...
    # TestConv2dLayer类的测试basicpatchembedding
    def test_basic_patch_embedding(self):
        ...
        self.assertFalse(layer.enable_linear)  # 断言为假
    ...
    # TestConv2dLayer类的测试enablelinear
    def test_enable_linear(self):
        ...
        self.assertTrue(layer.enable_linear)  # 断言为真
        _copy_weights(layer, ref)
    ...
    # TestConv2dLayer类的测试convpathwithpadding
    ...
    # TestConv2dLayer类的测试mulmatmatchesconv
    ...
    # TestConv2dLayer类的测试forwardcudausesmulmatwhenenabled
    ...
    # TestConv2dLayer类的测试forwardcudausesconvwhennoteligible
    ...
    # TestConv2dLayer类的测试tuplekernelsize
...
class TestConvValidation(unittest.TestCase):
    ...
    # TestConvValidation类的测试invalidpaddingmode
    def test_invalid_padding_mode(self):
        with self.assertRaises(ValueError):  # 断言抛出异常
            Conv3dLayer(3, 64, kernel_size=3, stride=1, padding_mode="invalid")

    # TestConvValidation类的测试conv3dinchannelsnotdivisiblebygroups
...
class TestConv3dLayer(unittest.TestCase):
    ...
    # TestConv3dLayer类的测试basictemporalpatchembedding
    ...
    # TestConv3dLayer类的测试mulmatmatchesconv
    def test_mulmat_matches_conv(self):
        layer = Conv3dLayer(
    ...
    # TestConv3dLayer类的测试nonmatchingstridedisableslinear
    def test_non_matching_stride_disables_linear(self):
        layer = Conv3dLayer(3, 64, kernel_size=3, stride=1, padding=1)
        self.assertFalse(layer.enable_linear)  # 断言为假

    # TestConv3dLayer类的测试dilationdisableslinear
    def test_dilation_disables_linear(self):
        layer = Conv3dLayer(3, 64, kernel_size=3, stride=3, dilation=2)
        self.assertFalse(layer.enable_linear)  # 断言为假

    # TestConv3dLayer类的测试disablelinear
    ...
    # TestConv3dLayer类的测试convpathwithpadding
    def test_conv_path_with_padding(self):
    ...
    # TestConv3dLayer类的测试outputshape
    def test_output_shape(self):
        layer = Conv3dLayer(
            3, 1152, kernel_size=[2, 14, 14], stride=[2, 14, 14], bias=False
        )
        x = torch.randn(1, 3, 2, 14, 14)
        out = layer.forward_native(x)
        self.assertEqual(out.shape, (1, 1152, 1, 1, 1))  # 断言相等

    # TestConv3dLayer类的测试batchprocessing
    ...
    # TestConv3dLayer类的测试forwardnativeusesmulmatwheneligible
    ...
    # TestConv3dLayer类的测试paddingvalid
    def test_padding_valid(self):
        layer = Conv3dLayer(
            3, 64, kernel_size=[2, 4, 4], stride=[2, 4, 4], padding="valid"
        )
        self.assertTrue(layer.enable_linear)  # 断言为真
        self.assertEqual(layer.padding, (0, 0, 0))  # 断言相等

    # TestConv3dLayer类的测试weightshape
    def test_weight_shape(self):
        layer = Conv3dLayer(
            3, 1152, kernel_size=[2, 14, 14], stride=[2, 14, 14], bias=False
        )
        self.assertEqual(layer.weight.shape, (1152, 3, 2, 14, 14))  # 断言相等

    # TestConv3dLayer类的测试glm4vworkflow
