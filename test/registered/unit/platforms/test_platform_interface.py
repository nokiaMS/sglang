# 文件名: test_platform_interface.py - 平台接口
"""
...
def _make_device_mixin(enum, name, dtype):
    ...
    class M(DeviceMixin):
        ...
        # M类的get_device_total_memory
        def get_device_total_memory(self, device_id=0):
            return 10**9

        # M类的get_current_memory_usage
...
class _StubPlatform(SRTPlatform):
    ...
    # _StubPlatform类的get_device_total_memory
    def get_device_total_memory(self, device_id=0):
        return 10**9

    # _StubPlatform类的get_current_memory_usage
    def get_current_memory_usage(self, device=None):
        return 5 * 10**8

    # _StubPlatform类的get_default_attention_backend
    def get_default_attention_backend(self):
        return "flashinfer"

    # _StubPlatform类的get_graph_runner_cls
    def get_graph_runner_cls(self):
        return object

    # _StubPlatform类的get_mha_kv_pool_cls
    def get_mha_kv_pool_cls(self):
        return object

    # _StubPlatform类的get_mla_kv_pool_cls
    def get_mla_kv_pool_cls(self):
        return object

    # _StubPlatform类的get_dsa_kv_pool_cls
    def get_dsa_kv_pool_cls(self):
        return object

    # _StubPlatform类的get_paged_allocator_cls
    def get_paged_allocator_cls(self):
        return object

    # _StubPlatform类的get_piecewise_backend_cls
    def get_piecewise_backend_cls(self):
        return object


# 内部方法_make_platform_ep
...
class TestDeviceCapability(CustomTestCase):
    ...
    def test_as_version_str(self):
        self.assertEqual(DeviceCapability(major=9, minor=0).as_version_str(), "9.0")  # 断言相等
        self.assertEqual(DeviceCapability(major=8, minor=9).as_version_str(), "8.9")  # 断言相等

    # TestDeviceCapability类的测试toint
    def test_to_int(self):
        self.assertEqual(DeviceCapability(major=9, minor=0).to_int(), 90)  # 断言相等
        self.assertEqual(DeviceCapability(major=8, minor=9).to_int(), 89)  # 断言相等
        self.assertEqual(DeviceCapability(major=0, minor=0).to_int(), 0)  # 断言相等
    ...
    def test_platform_identity_methods(self):
        ...
        for enum_val, name, dtype, method in _PLATFORM_IDENTITY:
            with self.subTest(method=method, enum=enum_val.name):
                ...
                self.assertTrue(getattr(mixin, method)())  # 断言为真

    # TestDeviceMixin类的测试iscudaalike
    def test_is_cuda_alike(self):
        ...
        for enum_val, name, dtype, expected in _CUDA_ALIKE:
            with self.subTest(enum=enum_val.name):
                mixin = _make_device_mixin(enum_val, name, dtype)
                self.assertEqual(mixin.is_cuda_alike(), expected)  # 断言相等

    # TestDeviceMixin类的测试isoutoftree
    def test_is_out_of_tree(self):
        oot = _make_device_mixin(PlatformEnum.OOT, "custom", "custom")
        self.assertTrue(oot.is_out_of_tree())  # 断言为真
        cuda = _make_device_mixin(PlatformEnum.CUDA, "cuda", "cuda")
        self.assertFalse(cuda.is_out_of_tree())  # 断言为假

    @patch("platform.machine")

    # TestDeviceMixin类的测试getcpuarchitecture
    def test_get_cpu_architecture(self, mock_machine):
        ...
        for machine_str, expected in cases:
            with self.subTest(machine=machine_str):
                ...
                self.assertEqual(DeviceMixin.get_cpu_architecture(), expected)  # 断言相等
...
class TestSRTPlatform(CustomTestCase):
    ...
    def test_compile_backend_signature_compatibility(self):
        ...
        base = SRTPlatform()
        self.assertEqual(base.get_compile_backend(mode="npugraph_ex"), "inductor")  # 断言相等

    # TestSRTPlatform类的测试basedeviceidentitystaysunspecified
    def test_base_device_identity_stays_unspecified(self):
        """The abstract SRT base should not claim any concrete in-tree device."""
        base = SRTPlatform()
        self.assertFalse(base.is_cuda())  # 断言为假
        self.assertFalse(base.is_cuda_alike())  # 断言为假


# TestCudaDeviceMixin类
class TestCudaDeviceMixin(CustomTestCase):
    ...
    def test_default_get_device_returns_cuda_device(self):
        ...
        self.assertEqual(base.get_device(2), torch.device("cuda", 2))  # 断言相等

    # TestCudaDeviceMixin类的测试cudaplatformidentity
    def test_cuda_platform_identity(self):
        base = CudaSRTPlatform()
        self.assertTrue(base.is_cuda())  # 断言为真
        self.assertTrue(base.is_cuda_alike())  # 断言为真
        self.assertIsInstance(base, CudaDeviceMixin)

    @patch("torch.cuda.get_device_properties")

    # TestCudaDeviceMixin类的测试defaultgetdevicetotalmemoryusescuda
    ...
    # TestCudaDeviceMixin类的测试defaultgetdevicenameusescuda
    def test_default_get_device_name_uses_cuda(self, mock_get_device_name):
        base = CudaSRTPlatform()
        self.assertEqual(base.get_device_name(1), "NVIDIA H100")  # 断言相等
        mock_get_device_name.assert_called_once_with(1)

    @patch("torch.cuda.get_device_properties")

    # TestCudaDeviceMixin类的测试defaultgetdeviceuuidusescuda
    def test_default_get_device_uuid_uses_cuda(self, mock_get_device_properties):
        ...
        base = CudaSRTPlatform()
        self.assertEqual(base.get_device_uuid(1), "1234")  # 断言相等
        mock_get_device_properties.assert_called_once_with(1)

    @patch("torch.cuda.get_device_capability", return_value=(9, 0))

    # TestCudaDeviceMixin类的测试defaultgetdevicecapabilityusescuda
    def test_default_get_device_capability_uses_cuda(self, mock_get_device_capability):
        base = CudaSRTPlatform()
        self.assertEqual(base.get_device_capability(1), DeviceCapability(9, 0))  # 断言相等
        mock_get_device_capability.assert_called_once_with(1)

    @patch("torch.cuda.empty_cache")

    # TestCudaDeviceMixin类的测试defaultemptycacheusescuda
    ...
    # TestCudaDeviceMixin类的测试defaultsynchronizeusescuda
    ...
    # TestCudaDeviceMixin类的测试defaultgetavailablememoryusescuda
    def test_default_get_available_memory_uses_cuda(self, mock_mem_get_info):
        base = CudaSRTPlatform()
        self.assertEqual(base.get_available_memory(1), (123, 456))  # 断言相等
        mock_mem_get_info.assert_called_once_with(1)

    # TestCudaDeviceMixin类的测试defaultdistributedbackendisnccl
    def test_default_distributed_backend_is_nccl(self):
        base = CudaSRTPlatform()
        self.assertEqual(base.get_torch_distributed_backend_str(), "nccl")  # 断言相等
    ...
    # TestCudaDeviceMixin类的测试defaultseedeverythingseedscuda
    ...
    # TestCudaDeviceMixin类的测试cudasrtplatformcapabilities
    def test_cuda_srt_platform_capabilities(self):
        base = CudaSRTPlatform()
        self.assertTrue(base.supports_fp8())  # 断言为真
        self.assertTrue(base.support_cuda_graph())  # 断言为真
        self.assertTrue(base.support_piecewise_cuda_graph())  # 断言为真


# TestSRTPlatformOverrides类
class TestSRTPlatformOverrides(CustomTestCase):
    ...
    def test_custom_get_dispatch_key_name(self):
        ...
        class P(_StubPlatform):
            ...
            # P类的get_dispatch_key_name
            def get_dispatch_key_name(self):
                return "npu"

        self.assertEqual(P().get_dispatch_key_name(), "npu")  # 断言相等

    # TestSRTPlatformOverrides类的测试customgetcompilebackend
    def test_custom_get_compile_backend(self):
        ...
        class P(_StubPlatform):
            ...
            # P类的get_compile_backend
            def get_compile_backend(self, mode=None):
                return "inductor"

        self.assertEqual(P().get_compile_backend(mode="npugraph_ex"), "inductor")  # 断言相等
...
class TestResolvePlatformWithEnv(CustomTestCase):
    ...
    # TestResolvePlatformWithEnv类的测试selectedpluginactivates
    def test_selected_plugin_activates(self, mock_envs, mock_ep):
        ...
        with patch("sglang.srt.platforms._load_platform_class") as mock_load:
            ...
            self.assertEqual(result, mock_instance)  # 断言相等

    @patch("sglang.srt.platforms.entry_points")
    @patch("sglang.srt.platforms.envs")

    # TestResolvePlatformWithEnv类的测试selectedpluginnotfound
    ...
    # TestResolvePlatformWithEnv类的测试selectedpluginhardwareunavailable
    ...
    # TestResolvePlatformWithEnv类的测试selectedpluginloadexception
    ...
    # TestResolvePlatformWithEnv类的测试otherpluginsnotloaded
...
# TestResolvePlatformAutoDiscover类
class TestResolvePlatformAutoDiscover(CustomTestCase):
    ...
    # TestResolvePlatformAutoDiscover类的测试iscudaavailableexcludesrocm
    def test_is_cuda_available_excludes_rocm(self, mock_torch):
        ...
        self.assertFalse(plat_mod._is_cuda_available())  # 断言为假

    @patch("sglang.srt.platforms.load_plugins_by_group")
    @patch("sglang.srt.platforms.envs")

    # TestResolvePlatformAutoDiscover类的测试singlepluginactivates
    ...
    # TestResolvePlatformAutoDiscover类的测试nopluginactivatescudafallback
    ...
    # TestResolvePlatformAutoDiscover类的测试nopluginnocudaactivatesbasefallback
    ...
    # TestResolvePlatformAutoDiscover类的测试nopluginrocmdoesnotactivatecudafallback
    ...
    # TestResolvePlatformAutoDiscover类的测试multiplepluginsactivateraises
    ...
    # TestResolvePlatformAutoDiscover类的测试pluginexceptiondoesnotcrash
    ...
    # TestResolvePlatformAutoDiscover类的测试pluginreturnsnoneisskipped
...
class TestLoadPlatformClass(CustomTestCase):
    ...
    # TestLoadPlatformClass类的测试validsubclass
    def test_valid_subclass(self, mock_resolve):
        ...
        self.assertTrue(issubclass(result, SRTPlatform))  # 断言为真

    @patch("sglang.srt.platforms.pkgutil.resolve_name")

    # TestLoadPlatformClass类的测试nonsubclassraisestypeerror
    ...
    # TestLoadPlatformClass类的测试nontyperaisestypeerror
...
class TestCurrentPlatformLazyInit(CustomTestCase):
    ...
    # TestCurrentPlatformLazyInit类的测试清理
    def tearDown(self):
    ...
    # TestCurrentPlatformLazyInit类的测试firstaccesstriggersresolve
    ...
    # TestCurrentPlatformLazyInit类的测试subsequentaccessusescache
    ...
    # TestCurrentPlatformLazyInit类的测试otherattributeraiseserror
