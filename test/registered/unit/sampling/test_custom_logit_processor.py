# 文件名: test_custom_logit_processor.py - 自定义逻辑处理器
...
class TestCustomLogitProcessorSerialization(CustomTestCase):
    ...
    # TestCustomLogitProcessorSerialization类的测试tostrproducesvalidjson
    def test_to_str_produces_valid_json(self):
        ...
        data = json.loads(s)
        self.assertIn("callable", data)  # 断言包含
        self.assertIsInstance(data["callable"], str)

    # TestCustomLogitProcessorSerialization类的测试roundtripserialization
    ...
    # TestCustomLogitProcessorSerialization类的测试fromstriscached
    def test_from_str_is_cached(self):
        ...
        self.assertIs(cls1, cls2)  # 断言是同一对象


# DisallowedTokensLogitsProcessor
class TestDisallowedTokensLogitsProcessor(CustomTestCase):

    # TestDisallowedTokensLogitsProcessor类的测试初始化设置
    def setUp(self):
        self.processor = DisallowedTokensLogitsProcessor()

    # TestDisallowedTokensLogitsProcessor类的测试disallowedtokenssettoneginf
    def test_disallowed_tokens_set_to_neg_inf(self):
        ...
        self.assertTrue(torch.isinf(result[0, 2]) and result[0, 2] < 0)  # 断言为真
        self.assertTrue(torch.isinf(result[0, 5]) and result[0, 5] < 0)  # 断言为真
        self.assertTrue(torch.isinf(result[1, 2]) and result[1, 2] < 0)  # 断言为真

    # TestDisallowedTokensLogitsProcessor类的测试allowedtokensunchanged
    def test_allowed_tokens_unchanged(self):
        ...
        self.assertEqual(result[0, 4].item(), 1.0)  # 断言相等
        self.assertTrue(torch.isinf(result[0, 3]) and result[0, 3] < 0)  # 断言为真

    # TestDisallowedTokensLogitsProcessor类的测试mismatchedparamsraises
...
class TestThinkingBudgetLogitProcessor(CustomTestCase):
    ...
    # TestThinkingBudgetLogitProcessor类的测试初始化设置
    def setUp(self):
        self.processor = Qwen3ThinkingBudgetLogitProcessor()

    # TestThinkingBudgetLogitProcessor类的内部方法_logits
    def _logits(self, batch_size=1):
        return torch.zeros(batch_size, self.VOCAB)

    # TestThinkingBudgetLogitProcessor类的测试budgetnotexceedednochange
    def test_budget_not_exceeded_no_change(self):
        ...
        logits = self._logits()
        result = self.processor(logits, params)
        self.assertEqual(result[0, 0].item(), 0.0)  # unchanged  # 断言相等

    # TestThinkingBudgetLogitProcessor类的测试budgetexceededforcesnewlinefirst
    def test_budget_exceeded_forces_newline_first(self):
        ...
        self.assertEqual(result[0, self.NL].item(), 0.0)  # 断言相等
        self.assertTrue(torch.isinf(result[0, 0]) and result[0, 0] < 0)  # 断言为真

    # TestThinkingBudgetLogitProcessor类的测试budgetexceededwithnewlineforcesendtoken
    def test_budget_exceeded_with_newline_forces_end_token(self):
        ...
        logits = self._logits()
        result = self.processor(logits, params)
        self.assertEqual(result[0, self.END].item(), 0.0)  # 断言相等
        self.assertTrue(torch.isinf(result[0, 0]) and result[0, 0] < 0)  # 断言为真

    # TestThinkingBudgetLogitProcessor类的测试skipswhennotinthinking
    def test_skips_when_not_in_thinking(self):
        ...
        result = self.processor(logits, params)
        self.assertTrue(torch.equal(result, original))  # 断言为真

    # TestThinkingBudgetLogitProcessor类的测试skipswhenthinkingalreadyended
    ...
    # TestThinkingBudgetLogitProcessor类的测试skipswhenbudgetisnone
    ...
    # TestThinkingBudgetLogitProcessor类的测试skipswhenbudgetisnegative
    ...
    # TestThinkingBudgetLogitProcessor类的测试noneparamsreturnsunchanged
    ...
    # TestThinkingBudgetLogitProcessor类的测试emptyparamsreturnsunchanged
    ...
    # TestThinkingBudgetLogitProcessor类的测试budgetzeroforcesimmediateend
    ...
    # TestThinkingBudgetLogitProcessor类的测试noneparamdictinlistskipped
    def test_none_param_dict_in_list_skipped(self):
        ...
        self.assertEqual(result[1, self.NL].item(), 0.0)  # 断言相等
        self.assertTrue(torch.isinf(result[1, 0]) and result[1, 0] < 0)  # 断言为真

    # TestThinkingBudgetLogitProcessor类的测试multiplethinkingstartcountsfromfirst
    ...
    # TestThinkingBudgetLogitProcessor类的测试deepseekr1variantforcesend
    def test_deepseek_r1_variant_forces_end(self):
        ...
        self.assertEqual(result[0, NL].item(), 0.0)  # 断言相等
        self.assertTrue(torch.isinf(result[0, 0]) and result[0, 0] < 0)  # 断言为真
...
class TestDeepseekOCRNoRepeatNGramLogitProcessor(CustomTestCase):
    ...
    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试初始化设置
    def setUp(self):
        self.processor = DeepseekOCRNoRepeatNGramLogitProcessor()

    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的内部方法_logits
    def _logits(self, batch_size=1):
        return torch.zeros(batch_size, self.VOCAB)

    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试bansrepeatedbigrams
    ...
    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试nonrepeatedtokensunchanged
    def test_non_repeated_tokens_unchanged(self):
        ...
        self.assertEqual(result[0, 1].item(), 0.0)  # 断言相等

    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试windowsizelimitssearch
    def test_window_size_limits_search(self):
        ...
        self.assertEqual(result[0, 3].item(), 0.0)  # 断言相等

    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试whitelistprotectstokens
    ...
    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试ngramsizezeroskips
    ...
    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试windowsizezeroskips
    ...
    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试emptyparamsreturnsunchanged
    def test_empty_params_returns_unchanged(self):
    ...
    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试shortsequenceskips
    ...
    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试unigrammode
    def test_unigram_mode(self):
        ...
        self.assertTrue(torch.isinf(result[0, 5]) and result[0, 5] < 0)  # 断言为真
        self.assertTrue(torch.isinf(result[0, 10]) and result[0, 10] < 0)  # 断言为真
        self.assertTrue(torch.isinf(result[0, 15]) and result[0, 15] < 0)  # 断言为真
        # Other tokens should be fine
        self.assertEqual(result[0, 0].item(), 0.0)  # 断言相等

    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试nonereqskips
    ...
    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试invalidngramsizetypeskips
    ...
    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试falsyparamsinlistskipped
    def test_falsy_params_in_list_skipped(self):
        ...
        self.assertTrue(torch.isinf(result[1, 1]) and result[1, 1] < 0)  # 断言为真

    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试searchendleqsearchstartskips
    ...
    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试invalidwhitelisttypehandled
    def test_invalid_whitelist_type_handled(self):
        ...
        self.assertTrue(torch.isinf(result[0, 1]) and result[0, 1] < 0)  # 断言为真

    # TestDeepseekOCRNoRepeatNGramLogitProcessor类的测试batchprocessing
    def test_batch_processing(self):
        ...
        self.assertEqual(result[1, 3].item(), 0.0)  # 断言相等
        self.assertEqual(result[1, 4].item(), 0.0)  # 断言相等
