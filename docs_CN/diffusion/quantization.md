...
SGLang-扩散模型 supports quantized transformer checkpoints. In most cases, keep
...
| `nunchaku-svdq`   | Pre-quantized Nunchaku transformer weights, usually named `svdq-{int4\|fp4}_r{rank}-...`   | `--transformer-weights-path`                                           | 模型-specific support such as Qwen-Image, FLUX, and Z-Image | `nunchaku`                            | SGLang can infer precision and rank from the filename and supports both `int4` and `nvfp4`                                             |
| `msmodelslim`     | Pre-quantized msmodelslim transformer weights                                              | `--model-path`                                                         | Wan2.2 family                           | None                                  | 目前 only compatible with the Ascend NPU family and supports both `w8a8` and `w4a4`                                               |
...
MXFP4 provides aggressive 4-bit compression with online quantization. **注意: Requires ROCm and MI350+ (gfx95x) GPU.**
...
sglang generate \
  ...
  --save-output
```
**注意:** Requires `aiter` package with MXFP4 kernel support

### Skipping Layers

默认情况下, online quantization quantizes every linear layer in
...
| Quant Algo | Base 模型 | Preferred CLI | HF Repo | Current Scope | Notes |
| --- | --- | --- | --- | --- | --- |
...
- To force the diffusion ModelOpt FP4 path onto a different FlashInfer
  backend, set `SGLANG_DIFFUSION_FLASHINFER_FP4_GEMM_BACKEND`. 已支持 values
...
安装 the runtime dependency first:
...
- **Multimodal_sd quantization**
  ...
  > 注意: SGLang does not support quantized embeddings, please disable this option when quantizing using msmodelslim.
