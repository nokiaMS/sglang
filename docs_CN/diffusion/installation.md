...
你可以 install SGLang-扩散模型 using one of the methods below. The standard installation already includes SGLang's optimized kernel stack, including both `sgl-kernel` and JIT kernels used by diffusion workloads.

## Standard 安装 (NVIDIA GPUs)

### Method 1: With pip or uv

建议 to use uv for a faster installation:
