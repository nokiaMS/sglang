...
你可以 also pass the overlay repo itself as `--model-path` if it contains `_overlay/overlay_manifest.json`.
...
For request and response examples, see [兼容 OpenAI 的 API](openai_api.md).
...
For frame interpolation and upscaling, see [后处理](post_processing.md).
...
See [量化](../quantization.md) for supported quantization families and examples.
...
For diffusers pipelines, 缓存-DiT can be enabled with `SGLANG_CACHE_DIT_ENABLED=true` or `--cache-dit-config`. See [缓存-DiT](../performance/cache/cache_dit.md).

### Layerwise Offload

Use layerwise offload when a large component does not fit comfortably in GPU memory. 默认情况下, `--dit-layerwise-offload` only applies to legacy DiT components. Use `--layerwise-offload-components` to select pipeline component names explicitly (`--layerwise-offload-modules` is accepted as an alias):
...
SGLang 扩散模型 can upload generated images and videos to S3-compatible object storage after generation.
...
See [环境变量](../environment_variables.md) for the full set of storage options.
...
你可以 also pass dotted CLI entries:
...
| `--cache-dit-config` | `{PATH}` | 缓存-DiT config for diffusers pipelines |
