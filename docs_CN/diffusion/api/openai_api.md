...
Loads one or more LoRA adapters and applies them to the model. 默认情况下, regular weights are statically merged, while FSDP-sharded weights use dynamic LoRA to avoid full-gather memory peaks.
...
- **`output-quality`** (string, optional): Preset quality level that automatically sets compression. **默认值 is `"default"`**. Valid values:
...
- **`output-compression`** (integer, optional): Direct compression level override (0-100). **默认值 is `None`**. When provided (not `None`), takes precedence over `output-quality`.
...
- **Format 支持**: Quality settings apply to JPEG, and video formats. PNG uses lossless compression and ignores these settings
