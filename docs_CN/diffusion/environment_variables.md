...
| `SGLANG_DIFFUSION_LOGGING_LEVEL` | `INFO` | 默认值 logging level |
...
These variables configure caching acceleration for 扩散模型 Transformer (DiT) models.
SGLang 支持 multiple caching strategies - see [caching documentation](performance/cache/index.md) for an overview.
...
| Environment Variable                | 默认值 | 说明                              |
|-------------------------------------|---------|------------------------------------------|
| `SGLANG_CACHE_DIT_ENABLED`          | false   | Enable 缓存-DiT acceleration            |
