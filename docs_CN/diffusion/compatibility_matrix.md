...
| 模型 Name                   | Hugging Face 模型 ID                             | Resolutions          | TeaCache | Sliding Tile Attn | Sage Attn | Video Sparse Attention (VSA) | Sparse Linear Attention (SLA) | Sage Sparse Linear Attention (SageSLA) | Sparse Video Gen 2 (SVG2) | Laser Attn | Block Sparse Attn | Rain Fusion Attn |
|:-----------------------------|:--------------------------------------------------|:---------------------|:--------:|:-----------------:|:---------:|:----------------------------:|:-----------------------------:|:--------------------------------------:|:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:|
...
2. SageSLA is based on SpargeAttn. 安装 it first with `pip install git+https://github.com/thu-ml/SpargeAttn.git --no-build-isolation`
...
4. LTX-2 / LTX-2.3 two-stage also supports `--ltx2-two-stage-device-mode {original,snapshot,resident}`:
   ...
   - 示例 (one prior run): `original` `154.67s`, `snapshot` `114.05s`, `resident` `75.71s`; peak VRAM trend is `original < snapshot < resident`.
...
SGLang 扩散模型 supports overriding individual pipeline components with
...
The table below lists concrete Hugging Face component repos that are already used in SGLang 扩散模型 docs or tests. It is not an exhaustive catalog of all compatible component repos.

| Base 模型                     | Override Key  | 示例 Repo                             | Notes                                     |
|:-------------------------------|:--------------|:-----------------------------------------|:------------------------------------------|
...
This section lists example LoRAs that have been explicitly tested and verified with each base model in the **SGLang 扩散模型** pipeline.
...
- 目前, only Hopper GPUs (H100s) are supported.
