...
本文档 describes the attention backends available in sglang diffusion (`sglang.multimodal_gen`) and how to select them.
...
后端 selection is performed by the shared attention layers (e.g. `LocalAttention` / `USPAttention` / `UlyssesAttention` in `sglang.multimodal_gen.runtime.layers.attention.layer`) and therefore applies to any model component using these layers (e.g. diffusion transformer / DiT and encoders).
...
| `sla_attn` | `SLA_ATTN` | Sparse Linear Attention. Requires `SpargeAttn`. 安装 with `pip install git+https://github.com/thu-ml/SpargeAttn.git --no-build-isolation`. |
...
Some backends require additional configuration. 你可以 pass these parameters via `--attention-backend-config`. This argument accepts:
...
| 后端 | CUDA | ROCm | XPU | MUSA | MPS | NPU | Notes |
|---|---:|---:|---:|---:|---:|---:|---|
