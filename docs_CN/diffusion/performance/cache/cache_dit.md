...
SGLang integrates [缓存-DiT](https://github.com/vipshop/cache-dit), a caching acceleration engine for 扩散模型 Transformers (DiT), to achieve up to **1.69x inference speedup** with minimal quality loss.

## 概览

**缓存-DiT** uses intelligent caching strategies to skip redundant computation in the denoising loop:

- **DBCache (Dual Block 缓存)**: Dynamically decides when to cache transformer blocks based on residual differences
...
Enable 缓存-DiT by exporting the environment variable and using `sglang generate` or `sglang serve` :
...
缓存-DiT supports loading acceleration configs from a custom YAML file. For
...
- DBCache + TaylorSeer + SCM (Step Computation Mask) + 缓存 CFG
...
Then apply the distributed config with: (注意: please add `--num-gpus N` to specify the number of gpus for distributed inference)
...
你可以 also define a 2D parallelism config yaml `parallel_2d.yaml` file that contains:
...
你可以 also define a 3D parallelism config yaml `parallel_3d.yaml` file that contains:
...
要启用 Ulysses Anything Attention, you can define a parallelism config yaml `parallel_uaa.yaml` file that contains:
...
For device that don't have NVLink support, you can enable Ulysses FP8 Communication to further reduce the communication overhead. 你可以 define a parallelism config yaml `parallel_fp8.yaml` file that contains:
...
你可以 also enable async ulysses CP to overlap the communication and computation. Define a parallelism config yaml `parallel_async.yaml` file that contains:
...
你可以 also specify the extra parallel modules in the yaml config. 例如, define a parallelism config yaml `parallel_extra.yaml` file that contains:
...
你可以 also specify the quantization config in the yaml file, required `torchao>=0.16.0`. 例如, define a yaml file `quantize.yaml` that contains:
...
```
Then, apply the quantization config from yaml. Please also enable torch.compile for better performance if you are using quantization. 例如:
...
### Combined Configs: 缓存 + Parallelism + 量化

你可以 also combine all the above configs together in a single yaml file `combined.yaml` that contains:
...
All 缓存-DiT parameters can be configured via environment variables.
See [环境变量](../../environment_variables.md) for the complete list.

## 支持的模型

SGLang 扩散模型 x 缓存-DiT supports almost all models originally supported in SGLang 扩散模型:
...
- **SGLang-native pipelines**: Distributed support (TP/SP) is not yet validated; 缓存-DiT will be automatically
  disabled when `world_size > 1`.
- **SCM minimum steps**: SCM requires >= 8 inference steps to be effective
- **模型 support**: Only models registered in 缓存-DiT's BlockAdapterRegister are supported
