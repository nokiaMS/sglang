...
本文档 is for deploying a RoCE network-based SGLang two-node inference service on a Kubernetes (K8S) cluster.
...
2. 请确保 your K8S cluster has LWS correctly installed. If it hasn't been set up yet, please follow the [installation instructions](https://github.com/kubernetes-sigs/lws/blob/main/site/content/en/docs/installation/_index.md). **注意:** For LWS versions ≤0.5.x, you must use the Downward API to obtain `LWS_WORKER_INDEX`, as native support for this feature was introduced in v0.6.0.

## Basic example

For the basic example documentation, refer to [部署 Distributed 推理 Service with SGLang and LWS on GPUs](https://github.com/kubernetes-sigs/lws/tree/main/docs/examples/sglang).

However, that document only covers the basic NCCL socket mode.

本节中, we’ll make some simple modifications to adapt the setup to the RDMA scenario.
...
你可以 use the command `kubectl logs -f sglang-0` to view the logs of the leader node.
