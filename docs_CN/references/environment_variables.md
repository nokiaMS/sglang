...
SGLang 支持 various environment variables that can be used to configure its runtime behavior. 本文档 provides a comprehensive list and aims to stay updated over time.

*注意: SGLang uses two prefixes for environment variables: `SGL_` and `SGLANG_`. This is likely due to historical reasons. While both are currently supported for different settings, future versions might consolidate them.*

## General 配置

| Environment Variable                      | 说明                                                                                                                      | 默认值 Value                |
|-------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|------------------------------|
...
| `SGLANG_CACHE_DIR`                        | 缓存 directory for model weights and other data | `~/.cache/sglang` |
...
| `SGLANG_SCHEDULER_RECV_SKIPPER_WEIGHT_DEFAULT` | 默认值 weight value for scheduler recv skipper counter (used when forward mode doesn't match specific modes). Only active when `--scheduler-recv-interval > 1`. The counter accumulates weights and triggers request polling when reaching the interval threshold. | `1000` |
...
## DeepGEMM 配置 (Advanced Optimization)
...
| `SGLANG_DETOKENIZER_MAX_STATES` | Maximum states for detokenizer | 默认值 value based on system |
...
| `SGLANG_QUANT_ALLOW_DOWNCASTING` | Allow weight dtype downcasting during loading (e.g., fp32 → fp16). 默认情况下, SGLang rejects this kind of downcasting when using quantization. | `false` |
| `SGLANG_FP8_IGNORED_LAYERS` | A comma-separated list of layer names to ignore during FP8 quantization. 例如: `model.layers.0,model.layers.1.,qkv_proj`. | `""` |
...
## PD 分离 — Staging Buffer (Heterogeneous TP)
