...
# Production 请求 Tracing

SGLang exports request trace data based on the OpenTelemetry Collector. 你可以 enable tracing by adding the `--enable-trace` and configure the OpenTelemetry Collector endpoint using `--otlp-traces-endpoint` when launching the server.

你可以 find example screenshots of the visualization in https://github.com/sgl-project/sglang/issues/8965.

## Setup Guide
This section explains how to configure the request tracing and export the trace data.
1. 安装 the required packages and tools
...
3. Start your SGLang server with tracing enabled
    ...
    要使用 the HTTP/protobuf span exporter, set the following environment variable and point to an HTTP endpoint, for example, `http://0.0.0.0:4318/v1/traces`.
    ```bash
...
6. Dynamically adjust trace level
    ...
    **注意**: You must set the parameter `--enable-trace`; otherwise, the trace capability will not be enabled regardless of any dynamic adjustments to the trace level.
...
**All of the following implementations are done in python/sglang/srt/observability/req_time_stats.py. 如果你想要 add another slice, please do it here.**
...
2. Create a trace context for a request
    Each request needs to call `TraceReqContext()` to initialize a request context, which is used to generate slice spans and record request stage info. 你可以 either store it within the request object or maintain it as a global variable.
...
## How to Extend the Tracing Framework to 支持 Complex Tracing Scenarios
