...
# 故障排查 and Frequently Asked Questions

## 故障排查

本页面 lists common errors and tips for resolving them.
...
- 你可以 also decrease `--mem-fraction-static` to a smaller value, such as 0.8 or 0.7. This decreases the memory usage of the KV cache memory pool and helps prevent OOM errors during both prefill and decoding. However, it limits maximum concurrency and reduces peak throughput.
...
### CUDA Error: Illegal 内存 Access Encountered
...
- If it is an out-of-memory issue, it may sometimes be reported as this error instead of "Out of 内存." Refer to the section above for guidance on avoiding OOM issues.
