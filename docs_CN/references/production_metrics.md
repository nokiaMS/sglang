...
SGLang exposes the following metrics via Prometheus. 你可以 enable it by adding `--enable-metrics` when you launch the server.
...
1.  **Start your SGLang server with metrics enabled:**
    ...
    python -m sglang.launch_server \
      ...
      --enable-mfu-metrics
    ```
    Replace `<your_model_path>` with the actual path to your model (e.g., `meta-llama/Meta-Llama-3.1-8B-Instruct`). Ensure the server is accessible from the monitoring stack (you might need `--host 0.0.0.0` if running in Docker). 默认情况下, the metrics endpoint will be available at `http://<sglang_server_host>:30000/metrics`.
...
*   **No Data on Dashboard:**
    *   Generate some traffic to your SGLang server to produce metrics. 例如, run a benchmark:
        ```bash
...
你可以 customize the setup by modifying these files. For instance, you might need to update the `static_configs` target in `prometheus.yaml` if your SGLang server runs on a different host or port.
