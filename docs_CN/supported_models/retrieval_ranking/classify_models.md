...
本文档 describes the `/v1/classify` API endpoint implementation in SGLang, which is compatible with vLLM's classification API format.
...
- `rid` (string, optional): 请求 ID for tracking
- `priority` (integer, optional): 请求 priority
...
**注意**: The `/classify` endpoint in SGLang was originally designed for reward models but now supports all non-generative models. Our `/v1/classify` endpoint provides a standardized vLLM-compatible interface for classification tasks.
...
- `500 Internal Server Error`: 服务器-side processing error
...
1. **Rust 模型 Gateway**: Handles routing and request/response models in `sgl-model-gateway/src/protocols/spec.rs`
2. **Python HTTP 服务器**: Implements the actual endpoint in `python/sglang/srt/entrypoints/http_server.py`
