...
| 模型 Family (Rerank)                          | 示例 HuggingFace Identifier       | Chat Template | 说明                                                                                                                      |
|------------------------------------------------|--------------------------------------|---------------|----------------------------------------------------------------------------------------------------------------------------------|
| **BGE-Reranker (BgeRerankModel)**              | `BAAI/bge-reranker-v2-m3`            | N/A           | 目前 only support `attention-backend` `triton` and `torch_native`. High-performance cross-encoder reranker model from BAAI. Suitable for reranking search results based on semantic relevance.   |
...
- `model` (required): 模型 to use for reranking
...
### 示例 客户端 请求 (supports optional instruct, top_n, and return_documents)
...
示例 (with `return_documents: false`):
...
- **注意**: 目前 only `Qwen3-VL-Reranker-2B` is tested and supported. The 8B model may have different behavior and is not guaranteed to work with this template.
