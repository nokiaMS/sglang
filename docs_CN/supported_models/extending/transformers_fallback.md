...
默认情况下, we will use sglang implementation if it is available. Otherwise, we will fall back to transformers one. However, you can switch the implementation by setting `--model-impl` to `transformers`.
...
Transformers fall back has supported most of available quantization in SGLang (except GGUF). See [量化 page](../../advanced_features/quantization.md) for more information about supported quantization in SGLang.
