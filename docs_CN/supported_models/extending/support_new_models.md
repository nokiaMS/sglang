...
本文档 explains how to add support for new language models and multimodal large language models (MLLMs) in
SGLang. It also covers how to test new models and register external implementations.

## How to 支持 a New Language 模型

To support a new model in SGLang, you only need to add a single file under
the [SGLang 模型 Directory](https://github.com/sgl-project/sglang/tree/main/python/sglang/srt/models). 你可以 learn
...
to [port a 模型 from vLLM to SGLang](#port-a-model-from-vllm-to-sglang)

## How to 支持 a New Multimodal Large Language 模型
...
你可以 refer to [Qwen2VL](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/models/qwen2_vl.py) or
...
For interactive debugging, compare the outputs of Hugging Face/Transformers and SGLang. 以下 two commands
...
- **(Required) MMMU**: follow MMMU benchmark [README.md](https://github.com/sgl-project/sglang/blob/main/benchmark/mmmu/README.md) to get SGLang vs. HF Transformer accuracy comparison. The accuracy score from SGLang run should not be much lower than that from HF Transformer run. Similarly, follow /docs_CN/developer_guide/benchmark_and_profiling.md to get performance comparison: TTFT and throughput must meet or exceed baselines (e.g., HF Transformer).
- **(Optional) Other evals**: If you ran other evals, please note the results in PR description.

## Port a 模型 from vLLM to SGLang

The [vLLM 模型 Directory](https://github.com/vllm-project/vllm/tree/main/vllm/model_executor/models) is a valuable
...
注意: make sure you add your new model to the supported models list in the supported models documentation.

## Registering an External 模型 Implementation
...
## 示例: Implementing and 服务化 a Llama Wrapper 模型

Below is an introductory, step-by-step walkthrough on how to implement a new model end-to-end in SGLang and then run it via the [Offline Engine](/docs_CN/basic_usage/offline_engine_api.ipynb).
...
注意 that almost all model implementations take in `config` and `quant_config` as arguments for their `__init__` method; `config` and `quant_config` are passed in via [`model_loader/loader.py`](https://github.com/sgl-project/sglang/blob/bf72b80122fd888bf619d17b96fa3e323ab809fc/python/sglang/srt/model_loader/loader.py#L219).
...
注意 that the signature for `forward` is essentially the same for any model; you can take a look at the other models defined in the [`models` directory](https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/models/) for references.
...
### 服务化 Our 模型 Via SGLang's Offline Engine
...
Thus, to register our `LlamaWrapper`, we want to follow the steps in the above section titled "Registering an External 模型 Implementation".
...
## 服务化 External 模型 via the Standard CLI
...
例如, if you are implementing a Llama wrapper, add this line at the end of your model file:
...
你可以 handle this by setting two additional environment variables:
...
例如, let's build a custom model based on Qwen2-VL-Instruct that takes the square root of the logits.
...
**注意:** you don't need a separate `EntryClass` for the custom processor as long as you associate the processor with the specific model class.

安装 the package, update `config.json`, and launch:
