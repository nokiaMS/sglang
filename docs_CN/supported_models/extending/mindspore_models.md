...
目前, the following models are supported:
...
> **注意**: 目前, MindSpore models are provided by an independent package `sgl-mindspore`. 支持 for MindSpore is built upon current SGLang support for Ascend NPU platform. Please first [install SGLang for Ascend NPU](../../platforms/ascend/ascend_npu.md) and then install `sgl-mindspore`:
