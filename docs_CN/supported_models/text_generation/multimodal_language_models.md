...
> See the [OpenAI APIs section](/docs_CN/basic_usage/openai_api_vision.md) for how to send multimodal requests.

## 已支持 models

Below the supported models are summarized in a table.

If you are unsure if a specific architecture is implemented, you can search for it via GitHub. 例如, to search for `Qwen2_5_VLForConditionalGeneration`, use the expression:
...
| 模型 Family (Variants)    | 示例 HuggingFace Identifier             | 说明                                                                                                                                                                                                     | Notes |
|----------------------------|--------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------|
...
| **Ernie4.5-VL** | `baidu/ERNIE-4.5-VL-28B-A3B-PT`              | Baidu's vision-language models(28B,424B). 支持 image and video comprehension, and also support thinking.                                                                     |  |
...
| **Qwen3-Omni** | `Qwen/Qwen3-Omni-30B-A3B-Instruct` |  Alibaba's omni-modal MoE model. 目前 supports the **Thinker** component (multimodal understanding for text, images, audio, and video), while the **Talker** component (audio generation) is not yet supported. |  |
...
SGLang 支持 audio-only ASR models via the OpenAI-compatible `/v1/audio/transcriptions` endpoint. Upload an audio file and receive a transcription.
...
| **NVIDIA Nemotron Nano 2.0 VL** | `nvidia/NVIDIA-Nemotron-Nano-12B-v2-VL-BF16` | The processor samples at 2 FPS, at a max of 128 frames, as per model training. The model uses [EVS](../../../python/sglang/srt/multimodal/evs/README.md), a pruning method that removes redundant tokens from video embeddings. 默认情况下 `video_pruning_rate=0.7`. Change this by providing: `--json-model-override-args '{"video_pruning_rate": 0.0}'` to disable EVS, for example. |
...
示例 OpenAI-compatible request that sends a video clip:
...
- **默认值 behavior**: Multimodal feature tensors are moved to CPU after processing to save GPU memory
- **With `--keep-mm-feature-on-device`**: 功能 tensors remain on GPU, reducing device-to-host copy overhead and improving latency, but consuming more GPU memory
...
### Bidirectional Attention in Multimodal 模型 服务化
**注意 for serving the Gemma-3 multimodal model**:

As mentioned in [Welcome Gemma 3: Google's all new multimodal, multilingual, long context open LLM
](https://huggingface.co/blog/gemma3#multimodality), Gemma-3 employs bidirectional attention between image tokens during the prefill phase. 目前, SGLang only supports bidirectional attention when using the Triton 注意力后端. 注意, however, that SGLang's current bidirectional attention implementation is incompatible with both CUDA Graph and Chunked Prefill.

要启用 bidirectional attention, you can use the `TritonAttnBackend` while disabling CUDA Graph and Chunked Prefill. 示例 launch command:
```shell
