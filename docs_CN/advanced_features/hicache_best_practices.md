...
- Besides configuring `--hicache-storage-backend` at startup, SGLang also supports **runtime attach/detach** of the HiCache storage backend (no restart required) via HTTP admin endpoints. See [Runtime Attach/Detach HiCache Storage 后端](hicache_storage_runtime_attach_detach.md).

## Key Configurations with Storage Backends Enabled

### 内存 Layout Optimization
...
### Heterogeneous TP 支持 (GQA/MHA models)
...
### Integration with PD 分离

HiCache works seamlessly with PD 分离. 你可以 choose between two configurations:
...
Here is an example of deploying DeepSeek-R1 with HiCache-HF3FS. 更多详细信息, see the [HF3FS Documentation](../../python/sglang/srt/mem_cache/storage/hf3fs/docs/README.md).
...
Here is an example of deploying Qwen3-235B-A22B-Instruct-2507 with Mooncake. 更多详细信息, see the [Mooncake Documentation](../../python/sglang/srt/mem_cache/storage/mooncake_store/README.md).
...
*本文档 will be continuously updated based on community feedback and new features. Contributions and suggestions are welcome!*
