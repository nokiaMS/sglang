...
SGLang 支持 a large variety of attention backends. Each of them has different pros and cons.
你可以 test them according to your needs.
...
| **后端**                     | **Page Size > 1 (native)** | **FP8 KV 缓存** | **FP4 KV 缓存** | **Spec topk=1** | **Spec topk>1** | **Sliding Window** | **MultiModal** |
|---------------------------------|-----------------------------|------------------|-----------------|-----------------|-----------------|--------------------|----------------|
| **FlashInfer**                  | ✅                          | ✅               | ❌              | ✅              | ✅              | ✅                 | ❌             |
...
| **后端**                | **Native Page Sizes**     | **FP8 KV 缓存** | **FP4 KV 缓存** | **Chunked 前缀缓存** | **Spec topk=1** | **Spec topk>1** |
|----------------------------|---------------------------|------------------|------------------|--------------------------|-----------------|-----------------|
| **FlashInfer MLA**         | 1                         | ❌               | ✅               | ✅                       | ✅              | ❌              |
| **FlashMLA**               | 64                        | ✅               | ✅               | ✅                       | ✅              | ❌              |
| **Cutlass MLA**            | 128                       | ✅               | ✅               | ✅                       | ✅              | ❌              |
...
The GDN linear attention layers have their own kernel backends, selected via `--linear-attn-backend` (default: `triton`). 你可以 override the kernel per phase with `--linear-attn-decode-backend` and `--linear-attn-prefill-backend`.

| **后端**              | **Decode** | **Prefill / Extend** | **Spec Decoding (Target Verify)** |
|--------------------------|------------|----------------------|-----------------------------------|
| **Triton (CUDA)**        | ✅         | ✅                   | ✅                                |
| **Triton (AMD/ROCm)**    | ✅         | ✅                   | ✅                                |
| **Triton (NPU)**         | ✅         | ✅                   | ❌                                |
...
Internally, the DSA backend dispatches to different sub-backends for prefill and decode phases. 你可以 override these with `--dsa-prefill-backend` and `--dsa-decode-backend` (the `--nsa-prefill-backend` / `--nsa-decode-backend` aliases are deprecated):

| **Sub-backend**       | **Prefill** | **Decode** | **Notes**                                     |
|-----------------------|-------------|------------|-----------------------------------------------|
| **flashmla_sparse**   | ✅          | ✅         | 默认值 prefill on Hopper and Blackwell (bf16) |
| **flashmla_kv**       | ✅          | ✅         | 默认值 decode for FP8 on Blackwell with DP   |
| **flashmla_auto**     | ✅          | ❌         | Auto-selects flashmla_sparse or flashmla_kv based on kv_cache_dtype |
| **fa3**               | ✅          | ✅         | 默认值 decode on Hopper (bf16)               |
| **trtllm**            | ✅          | ✅         | 默认值 decode on Blackwell (bf16); default for both on Blackwell without DP |
| **tilelang**          | ✅          | ✅         | 默认值 on AMD (ROCm)                         |
...
你可以 mix-and-match attention backends for prefill and decode. This is useful when one backend excels at prefill and another excels at decode. For the implementation details, please see `python/sglang/srt/layers/attention/hybrid_attn_backend.py`.
...
- FlashInfer (默认值 for Non-Hopper Machines, e.g., A100, A40)
```bash
python3 -m sglang.launch_server \
...
- FlashAttention 3 (默认值 for Hopper Machines, e.g., H100, H200, H20)
```bash
python3 -m sglang.launch_server \
  --model meta-llama/Meta-Llama-3.1-8B-Instruct \
...
- TRTLLM MLA with FP8 KV 缓存 (Higher concurrency, lower memory footprint)
```bash
python3 -m sglang.launch_server \
  --tp 8 \
  --model deepseek-ai/DeepSeek-R1 \
...
- TRTLLM MHA (XQA backend) (Optimized for SM90 and SM120, e.g., H20, H200, 5090)
  注意 that TRTLLM XQA backend only works well for pagesize 64.
```bash
python3 -m sglang.launch_server \
  --tp 4 \
  --model Qwen/Qwen3.5-35B-A3B-FP8 \
...
1. Run without cuda graph. 支持 the two forward functions
- forward_extend
