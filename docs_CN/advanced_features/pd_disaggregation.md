...
PD 分离 resolves these by separating the two stages, enabling tailored optimizations for each.
...
目前, we support Mooncake and NIXL as the transfer engine.

## 性能分析 in PD 分离 Mode

When you need to profile prefill or decode workers in PD disaggregation mode, please refer to the [Profile In PD 分离 Mode](/docs_CN/developer_guide/benchmark_and_profiling.md#profile-in-pd-disaggregation-mode) section in the 基准测试与性能分析 guide. Due to torch profiler limitations, prefill and decode workers must be profiled separately using dedicated command-line options.

## 路由器 Integration

For deploying PD disaggregation at scale with load balancing and fault tolerance, SGLang provides a router. The router can distribute requests between prefill and decode instances using various routing policies. For detailed information on setting up routing with PD disaggregation, including configuration options and deployment patterns, see the [SGLang 模型 Gateway (former 路由器)](../advanced_features/sgl_model_gateway.md#prefill-decode-disaggregation).
...
PD 分离 with Mooncake supports the following environment variables for fine-grained control over system behavior.

#### NVLink Transport 配置
要启用 NVLink transport for KV cache transfers with the mooncake backend (recommended for NVL72 deployments), set the following environment variables. 注意 that auxiliary data transfer will still use TCP as a temporary workaround.
...
The `SGLANG_MOONCAKE_CUSTOM_MEM_POOL` environment variable enables the custom memory pool. 已支持 values are `NVLINK` (or `True`), `BAREX`, and `INTRA_NODE_NVLINK`.
...
| **`SGLANG_DISAGGREGATION_WAITING_TIMEOUT`** | Timeout (seconds) for receiving KV 缓存 after request initialization | `300` |
...
> **注意:** The staging buffer is designed for non-MLA models (e.g. GQA, MHA). MLA models (e.g. DeepSeek-V2/V3) should not enable this flag.
...
默认情况下, NIXL uses the **UCX** backend for KV cache transfers. 你可以 select a different NIXL plugin backend depending on your infrastructure using the environment variable `SGLANG_DISAGGREGATION_NIXL_BACKEND`.
