...
### Selective Graph Breaks in 模型 Code
...
你可以 also insert a bare graph break (no computation) using the `break_graph()` helper:
...
要启用 breakable CUDA graph at the environment level (without debug mode), set the environment variable:
