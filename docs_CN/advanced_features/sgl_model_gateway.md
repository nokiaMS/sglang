...
SGLang 模型 Gateway is a high-performance model-routing gateway for large-scale LLM deployments. It centralizes worker lifecycle management, balances traffic across heterogeneous protocols (HTTP, gRPC, OpenAI-compatible), and provides enterprise-ready control over history storage, MCP tooling, and privacy-sensitive workflows. The gateway is deeply optimized for the SGLang serving runtime, but can route to any OpenAI-compatible backend.
...
3. [安装](#installation)
4. [快速开始](#quick-start)
5. [部署 Modes](#deployment-modes)
   - [Co-launch 路由器 and Workers](#co-launch-router-and-workers)
   - [Separate Launch (HTTP)](#separate-launch-http)
   - [gRPC Launch](#grpc-launch)
   - [Prefill-Decode 分离](#prefill-decode-disaggregation)
   - [OpenAI 后端 Proxy](#openai-backend-proxy)
   - [Multi-模型 推理 Gateway](#multi-model-inference-gateway)
6. [API 参考](#api-reference)
   - [推理 Endpoints](#inference-endpoints)
   - [Tokenization Endpoints](#tokenization-endpoints)
   - [Parser Endpoints](#parser-endpoints)
   - [Classification API](#classification-api)
   - [Conversation and 响应 APIs](#conversation-and-response-apis)
   - [工作进程 Management APIs](#worker-management-apis)
...
17. [安全 and Authentication](#security-and-authentication)
    - [TLS (HTTPS) for Gateway 服务器](#tls-https-for-gateway-server)
    - [mTLS for 工作进程 Communication](#mtls-for-worker-communication)
18. [可观测性](#observability)
    - [Prometheus Metrics](#prometheus-metrics)
    - [OpenTelemetry Tracing](#opentelemetry-tracing)
    - [Logging](#logging)
19. [Production Recommendations](#production-recommendations)
    - [安全 Best Practices](#security-best-practices)
    - [High Availability](#high-availability)
    - [性能](#performance)
    - [Kubernetes 部署](#kubernetes-deployment)
    - [Monitoring with PromQL](#monitoring-with-promql)
20. [配置 参考](#configuration-reference)
...
- **推理 Gateway Mode (`--enable-igw`)** dynamically instantiates multiple router stacks (HTTP regular/PD, gRPC) and applies per-model policies for multi-tenant deployments.
...
- **工作进程 Manager** discovers capabilities (`/server_info`, `/get_model_info`), tracks load, and registers/removes workers in the shared registry.
...
| `--eviction-interval-secs` | 120 | 缓存 eviction cadence in seconds |
...
| Parser ID | 模型 Family | Think Tokens |
|-----------|--------------|--------------|
...
The gateway provides native 模型 Context Protocol (MCP) client integration for tool execution.
...
| `logging/` | 请求 tracking headers and response modification |
...
**注意:** Rate limiting state is per-worker thread and not shared across gateway replicas. For production, consider implementing rate limiting at a shared layer (e.g., Redis)

---

## Language Bindings

SGLang 模型 Gateway provides official language bindings for Python and Go, enabling integration with different technology stacks and organizational requirements.
...
The Python bindings are used throughout this documentation. See the [快速开始](#quick-start) and [部署 Modes](#deployment-modes) sections for detailed examples.
...
| **CLI 支持** | Full CLI (smg, sglang-router) | Library only |
...
2. **路由器-only Authentication**: Clients authenticate to router
3. **工作进程-only Authentication**: 路由器 open, workers require keys
4. **Full Authentication**: Both router and workers protected

### TLS (HTTPS) for Gateway 服务器
...
**Key Points:**
- 客户端 certificate and key must be provided together
...
Gateway HTTPS + 工作进程 mTLS + API Key authentication:
...
| 路由器 | `smg_router_*` | `requests_total`, `request_duration_seconds`, `request_errors_total`, `stage_duration_seconds`, `upstream_responses_total` |
| 推理 | `smg_router_*` | `ttft_seconds`, `tpot_seconds`, `tokens_total`, `generation_duration_seconds` |
| 工作进程 | `smg_worker_*` | `pool_size`, `connections_active`, `requests_active`, `health_checks_total`, `selection_total`, `errors_total` |
...
This section provides guidance for deploying SGLang 模型 Gateway in production environments.
...
|-----------|----------------------|--------|
| 工作进程 Registry | No (independent) | Each replica discovers workers independently |
| Radix 缓存 Tree | No (independent) | 缓存 hits may decrease by 10-20% |
...
1. **Prefer horizontal scaling over vertical scaling**: 部署 multiple smaller gateway replicas rather than one large instance with excessive CPU and memory. This provides:
...
| Parameter | Type | 默认值 | 说明 |
|-----------|------|---------|-------------|
| `--host` | str | 127.0.0.1 | 路由器 host |
| `--port` | int | 30000 | 路由器 port |
| `--worker-urls` | list | [] | 工作进程 URLs (HTTP or gRPC) |
| `--policy` | str | cache_aware | Routing policy |
| `--max-concurrent-requests` | int | -1 | Concurrency limit (-1 disables) |
| `--request-timeout-secs` | int | 600 | 请求 timeout |
...
| `--worker-startup-timeout-secs` | int | 600 | 工作进程 init timeout |
...
| `--service-discovery-port` | int | 工作进程 port (default 80) |
...
| Parameter | Type | 说明 |
|-----------|------|-------------|
| `--tls-cert-path` | str | 服务器 certificate for gateway HTTPS (PEM) |
| `--tls-key-path` | str | 服务器 private key for gateway HTTPS (PEM) |
| `--client-cert-path` | str | 客户端 certificate for worker mTLS (PEM) |
| `--client-key-path` | str | 客户端 private key for worker mTLS (PEM) |
...
SGLang 模型 Gateway continues to evolve alongside the SGLang runtime. Keep CLI flags, integrations, and documentation aligned when adopting new features or contributing improvements.
