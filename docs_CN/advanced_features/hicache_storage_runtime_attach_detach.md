...
# Runtime Attach/Detach HiCache Storage 后端 (No Restart)

本文档 explains how to **dynamically attach/detach the HiCache L3 storage backend at runtime** (e.g., `mooncake` / `hf3fs` / `nixl` / `file` / `aibrix` / `eic`) while **SGLang is already running and serving traffic**, without restarting the process.
...
2. **TokenizerManager** (`python/sglang/srt/managers/tokenizer_control_mixin.py`)
   - Sends the request to the 调度器 via `FanOutCommunicator`
...
The 调度器 uses `is_fully_idle()` which checks:
...
> 提示: before switching, drain upstream traffic and wait for the server to become idle, then call attach/detach.
...
目前 there is **no automatic partial rollback** across DP ranks (see TODO in code). Operationally:
...
- `hicache_storage_backend_extra_config_json` can include both:
  - **后端 configuration** (e.g., Mooncake master/metadata/protocol, etc.)
