...
SGLang 支持 various quantization methods, including offline quantization and online dynamic quantization.
...
**注意: For better performance, usability and convenience, offline quantization is recommended over online quantization.**
...
以下 table summarizes quantization method support across NVIDIA and AMD GPUs, Ascend NPUs.
...
| `modelopt` / `modelopt_fp8` | Yes (Hopper/SM90+) | No | No | [NVIDIA ModelOpt](https://github.com/NVIDIA/模型-Optimizer); requires NVIDIA hardware |
| `modelopt_fp4` | Yes (Blackwell/SM100+) | No | No | [NVIDIA ModelOpt](https://github.com/NVIDIA/模型-Optimizer); native FP4 on Blackwell (B200, GB200) |
...
## GEMM Backends for FP4/FP8 量化

:::{note}
后端 selection is supported only for **blockwise FP8** and **NVFP4** GEMM. When running FP8 or FP4 quantized models, you can select the GEMM backend via `--fp8-gemm-backend` and `--fp4-gemm-backend`.
:::
...
downloaded Hugging Face or msModelSlim config. 例如, DeepSeek V3/R1 models are already in FP8, so do not add redundant parameters.**
...
We strongly suggest the use of Unsloth to quantize and load the model. 请参考 [SGLang 部署 & 推理 Guide with Unsloth](https://docs.unsloth.ai/basics/inference-and-deployment/sglang-guide).
...
- Command Line 用法 (Gaudi/CPU/Intel GPU/CUDA)
...
2. Limited 支持 for Quantized MoE 模型
...
3. Limited 支持 for Quantized VLMs
    <details>
...
#### Using [NVIDIA ModelOpt](https://github.com/NVIDIA/模型-Optimizer)

NVIDIA 模型 Optimizer (ModelOpt) provides advanced quantization techniques optimized for NVIDIA hardware.

**Offline vs. Online 量化:**

SGLang 支持 two modes for ModelOpt.

* **Offline 量化 (pre-quantized):**
    * **用法:** Download a pre-quantized model from Hugging Face or run `hf_ptq.py` once to create a new quantized checkpoint. Then load this quantized checkpoint.
...
* **Online 量化 (quant and serve):**
    * **用法:** Load a standard BF16/FP16 model and add a flag. The engine applies quantization *on startup*.
    * **Pros:** Convenient (no new checkpoint needed).
    * **Cons:** **High startup time**, increases VRAM usage during initialization (risk of OOM).

以下 sections guide you through using the Offline path: loading pre-quantized models or creating your own checkpoints.
...
If a pre-quantized checkpoint is not available for your model, you can create one using NVIDIA 模型 Optimizer's `hf_ptq.py` script.
...
`--qformat`: 量化 formats `fp8`, `nvfp4`, `nvfp4_mlp_only`

`--kv_cache_qformat`: KV cache quantization format (default: `fp8`)

**注意:** The default `kv_cache_qformat` may not be optimal for all use cases. Consider setting this explicitly.
...
For detailed usage and supported model architectures, see [NVIDIA 模型 Optimizer LLM PTQ](https://github.com/NVIDIA/模型-Optimizer/tree/main/examples/llm_ptq).
...
你可以 also use ModelOpt quantization programmatically:
...
- **Advanced 量化**: Supports cutting-edge FP8 and FP4 quantization techniques
...
- **LLM quantization**
    ...
    > 注意: 你可以 find pre-quantized validated models on [modelscope/Eco-Tech](https://modelscope.cn/models/Eco-Tech).
...
要启用 online quantization, you can simply specify `--quantization` in the command line. 例如, you can launch the server with the following command to enable `FP8` quantization for model `meta-llama/Meta-Llama-3.1-8B-Instruct`:
...
SGLang also supports quantization methods based on [torchao](https://github.com/pytorch/ao). 你可以 simply specify `--torchao-config` in the command line to support this feature. 例如, if you want to enable `int4wo-128` for model `meta-llama/Meta-Llama-3.1-8B-Instruct`, you can launch the server with the following command:
...
SGLang 支持 the following quantization methods based on torchao `["int8dq", "int8wo", "fp8wo", "fp8dq-per_tensor", "fp8dq-per_row", "int4wo-32", "int4wo-64", "int4wo-128", "int4wo-256"]`.

注意: According to [this issue](https://github.com/sgl-project/sglang/issues/2219#issuecomment-2561890230), `"int8dq"` method currently has some bugs when using together with cuda graph capture. So we suggest to disable cuda graph capture when using `"int8dq"` method. Namely, please use the following command:
...
SGLang-扩散模型 supports MXFP8 quantization for diffusion models (such as Wan2.2) on Ascend A5 NPUs, in both online and offline (ModelSlim) modes. This is separate from the LLM serving path and uses the `sglang serve` / `sglang generate` CLI.

**依赖要求:** Ascend A5, CANN ≥ 8.0.RC3
...
For the full quantization + format conversion workflow and a complete list of supported schemes, see [扩散模型 量化 on Ascend NPU](../platforms/ascend/ascend_npu_quantization.md#diffusion-model-quantization-on-ascend-npu) and [SGLang-扩散模型 量化](../diffusion/quantization.md#modelslim).

## 参考

- [GPTQModel](https://github.com/ModelCloud/GPTQModel)
- [LLM Compressor](https://github.com/vllm-project/llm-compressor/)
- [NVIDIA 模型 Optimizer (ModelOpt)](https://github.com/NVIDIA/模型-Optimizer)
- [NVIDIA 模型 Optimizer LLM PTQ](https://github.com/NVIDIA/模型-Optimizer/tree/main/examples/llm_ptq)
...
- [vLLM 量化](https://docs.vllm.ai/en/latest/quantization/)
