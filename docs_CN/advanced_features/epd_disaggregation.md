...
In modern Vision-Language 模型 (VLM) inference, request execution naturally decomposes into three distinct stages: Encoder, Prefill, and Decode.
...
你可以 launch a language-only model using `--language-only`, or an encoder-only model using `--encoder-only`.
When launching a language-only model, you must additionally specify the encoder service endpoints via `--encoder-urls`.

我们支持 multiple encoder transfer backends, including zmq_to_scheduler, zmq_to_tokenizer, and mooncake (the default is zmq_to_scheduler). The backend can be selected using `--encoder-transfer-backend`.
...
At a high level, the encoder checks whether the image embedding already exists in Mooncake. 缓存 hits are prefetched from the global store, while misses are encoded normally and inserted into the cache in the background.
...
你可以 run the encoder as a gRPC server while keeping prefill/decode as HTTP.
