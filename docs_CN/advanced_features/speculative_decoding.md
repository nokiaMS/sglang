...
- [Standalone 投机解码 (Small Draft 模型)](#standalone-speculative-decoding-small-draft-model)
- [投机解码 V2 (Overlap 调度器)](#speculative-decoding-v2-overlap-scheduler)
- [Ngram 投机解码](#ngram-speculative-decoding)
- [Full Parameter 参考](#full-parameter-reference)
- [OOM 故障排查](#oom-troubleshooting)
- [参考资料](#references)
...
- **模型 is MTP-enabled**: Use **MTP via speculative decoding** (often with small `speculative_num_steps/topk/num_draft_tokens`, see the example section).
...
要启用 EAGLE speculative decoding the following parameters are relevant:
...
| `--speculative-draft-model-quantization` | 量化 method for the draft model. Use `"unquant"` to force no quantization even when the target model is quantized. | Same as target model |
...
If you use EAGLE with `--speculative-eagle-topk 1` and your acceptance rate varies across requests, see [Adaptive 投机解码](adaptive_speculative_decoding.md).

你可以 find the best combinations of these parameters with [bench_speculative.py](https://github.com/sgl-project/sglang/blob/main/scripts/playground/bench_speculative.py).


### EAGLE-2 Decoding

你可以 enable EAGLE-2 Decoding by setting `--speculative-algorithm EAGLE` and choosing an appropriate model.
...
By employing a truncated high-frequency token vocabulary in the draft model, EAGLE speculative decoding reduces `lm_head` computational overhead while accelerating the pipeline without quality degradation. 更多详细信息, check out [the paper](https://arxiv.org/pdf/2502.14856).

In our implementation, set `--speculative-token-map` to enable the optimization. 你可以 get the high-frequency tokens in FR-Spec from [this model](https://huggingface.co/thunlp/LLaMA3-Instruct-8B-FR-Spec). Or you can obtain high-frequency tokens by directly downloading these tokens from [this repo](https://github.com/thunlp/FR-Spec/tree/main?tab=readme-ov-file#prepare-fr-spec-vocabulary-subset).
...
你可以 enable EAGLE-3 decoding by setting `--speculative-algorithm EAGLE3` and choosing an appropriate model.
...
我们支持 [MTP (Multi-Token Prediction)](https://arxiv.org/pdf/2404.19737) in SGLang by using speculative decoding. We use `XiaomiMiMo/MiMo-7B-RL` as an example here (for DeepSeek MTP usage, refer to [deepseek_v32 doc](../basic_usage/deepseek_v32.md#multi-token-prediction)).
...
## Standalone 投机解码 (Small Draft 模型)
...
| `--speculative-draft-model-quantization` | 量化 for the draft model. Use `"unquant"` to disable quantization on the draft even when the target is quantized. | Same as target |

> **注意:** Standalone speculative decoding currently **does not support** `--enable-dp-attention`.
...
SGLang provides an **experimental 投机解码 V2** implementation that enables an overlap scheduler and uses V2 speculative workers (e.g. `StandaloneWorkerV2`, `EAGLEWorkerV2`).

要启用 it, set the environment variable:
...
| `--speculative-ngram-capacity` | 缓存 capacity (number of entries). | `10,000,000` |
...
| `--speculative-draft-model-quantization` | `str` | Same as target | 量化 for the draft model (`"unquant"` to disable) |
...
| `--speculative-ngram-capacity` | `int` | `10,000,000` | 缓存 capacity |

### Environment variables

| Variable | 默认值 | 说明 |
|---|---|---|
| `SGLANG_ENABLE_SPEC_V2` | `False` | Enable 投机解码 V2 (overlap scheduler) |
...
> [!WARNING]
> **Out of 内存 (OOM)?** Speculative decoding may increase GPU memory usage because the draft tree, CUDA graphs, and verification-related buffers consume additional VRAM. If you encounter OOM errors, try the following adjustments.
...
This enhances drafting accuracy by operating on features instead of tokens for more regular inputs and by additionally passing tokens from the next timestep to reduce sampling randomness. 更多详细信息, see the [EAGLE-2](https://arxiv.org/abs/2406.16858) and [EAGLE-3](https://arxiv.org/abs/2503.01840) papers.
