...
At startup, SGLang pre-builds one runtime state per candidate tier. 默认情况下, the candidate tiers are `candidate_steps = [1, 3, 7]`.
...
如果你想要 override the defaults, add `--speculative-adaptive-config /path/to/adaptive_spec.json`.
...
你可以 inspect the active tier and acceptance metric via `/server_info`:
