...
更多详细信息, please refer to [Bench 服务化 Guide](./bench_serving.md).

### Profile In PD 分离 Mode
...
- Both options support multiple worker URLs for multi-instance setups:
  ...
  ```
- 请确保 `SGLANG_TORCH_PROFILER_DIR` is set on all worker nodes before starting the servers
- 更多详细信息 on setting up PD disaggregation, see [PD 分离 Guide](../advanced_features/pd_disaggregation.md)
...
你可以 do this by running `python3 -m sglang.profiler`. 例如:
...
你可以 also combine the above operations into a single command
...
The `/start_profile` endpoint starts profiling on the server. 你可以 control when profiling begins and how long it runs using the following parameters:
...
- `activities` (optional): List of activities to profile, e.g., `["CPU", "GPU"]`. 默认值 is `["CPU", "GPU"]`
- `merge_profiles` (optional): Whether to merge distributed traces. 默认值 is `false`

**注意 on step ranges:** 性能分析 starts at `start_step` (inclusive) and continues for `num_steps` iterations. 例如, with `start_step=3` and `num_steps=10`, profiling captures steps 3, 4, 5, 6, 7, 8, 9, 10, 11, and 12 (10 steps total, starting from step 3).
...
#### Multi-Node 性能分析 and Shared Storage Considerations
...
1. Prerequisite:
   ...
   安装 using apt, or run inside a [NVIDIA Docker container](https://catalog.ngc.nvidia.com/orgs/nvidia/containers/pytorch/tags) or [SGLang Docker container](https://github.com/sgl-project/sglang/tree/main/docker).
...
### Layer-wise NVTX 性能分析 with Nsight Systems
...
1. Launch the server with layerwise NVTX enabled under Nsight Systems:
   ...
   注意: NVTX markers are not emitted for kernel launches captured by CUDA graphs. Use `--disable-cuda-graph` to ensure all layerwise NVTX markers are emitted in the trace.
...
4. 性能分析 will automatically stop after 10 steps (due to `num_steps: 10`). If you hadn't specified `num_steps`, you would need to manually stop it:
...
- **内存 tracking**: Identify layers with large memory allocations
...
1. 你可以 benchmark a model using dummy weights by only providing the config.json file. This allows for quick testing of model variants without training. To do so, add `--load-format dummy` to the above commands and then you only need a correct `config.json` under the checkpoint folder.
2. 你可以 benchmark a model with modified configs (e.g., less layers) by using `--json-model-override-args`. 例如, you can benchmark a model with only 2 layers and 2 kv heads using:
...
3. 你可以 use `--python-backtrace=cuda` to see python call stack for all CUDA kernels, as in PyTorch Profiler. (Caveat: this can cause inaccurately long kernel runtimes for CUDA event based timing)
