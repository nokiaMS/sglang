...
要启用 code completion with `clangd`, run `python -m sglang.jit_kernel` to generate a `.clangd` configuration file in your current directory.
...
> 注意: `TensorMatcher` is a temporary expression and should not be stored in a variable.

> 提示: Add `//` at the end of the `TensorMatcher` chain to enforce proper indentation.
