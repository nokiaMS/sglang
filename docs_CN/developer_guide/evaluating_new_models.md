...
# Evaluating New 模型 with SGLang

本文档 provides commands for evaluating models' accuracy and performance. Before open-sourcing new models, we strongly suggest running these commands to verify whether the score matches your internal benchmark results.
...
性能 benchmarks measure **Latency** (Time To First Token - TTFT) and **Throughput** (tokens/second).

### LLMs

**Latency-Sensitive 基准测试**
...
你可以 also benchmark the performance of processing a single batch offline.
...
你可以 run more granular benchmarks:
...
3.  **Launch configuration**: 模型 path, TP size, and any special flags.
