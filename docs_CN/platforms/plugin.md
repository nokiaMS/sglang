...
> **注意**: `load_plugins()` is idempotent (guarded by `_plugins_loaded` flag). In spawn'd subprocesses the flag resets, so plugins are correctly re-loaded.
...
| Method | 默认值 | 说明 |
|---|---|---|
| `get_default_attention_backend()` | `raise NotImplementedError` | 默认值 attention backend name |
...
- **可观测性**: Add logging, metrics, and tracing to any function
- **Behavior modification**: Modify function arguments or return values
- **性能 profiling**: Add timing to critical functions
...
> **注意**: Only `REPLACE` accepts a class as the hook. Passing a class to `BEFORE`/`AFTER`/`AROUND` raises `TypeError` at registration time.
...
| `sglang.srt.managers.scheduler.Scheduler.run_batch` | 性能分析 / inspection |
