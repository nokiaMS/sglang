...
SGLang 支持 high-performance TPU inference through the SGLang-JAX backend, which is specifically optimized for Google Cloud TPUs. The JAX-based implementation delivers exceptional throughput and low latency for Large Language 模型 (LLM) serving workloads on TPU hardware.
...
| Radix Tree KV 缓存 | ✅ | 内存-efficient prefix sharing between requests |
| FlashAttention 后端 | ✅ | TPU-optimized attention kernel for long sequences |
| Tensor Parallelism | ✅ | Distribute models across multiple TPU cores |
| Paged Attention | ✅ | Flexible KV cache management with paging |
| 投机解码 (EAGLE/EAGLE3) | ✅ | 20-40% throughput improvement for compatible models |
| Chunked Prefill | ✅ | Mixed prefill-decode batching |
| 兼容 OpenAI 的 API | ✅ | Drop-in replacement for OpenAI API |
| Data Parallel Attention | 🚧 | In development - Attention computation with data parallelism |
| 量化 | 🚧 | In development - 模型 quantization for reduced memory usage |
| Multi-LoRA | 🚧 | In development - Serve multiple LoRA adapters simultaneously |

### 注意力后端 Comparison

| 后端 | Paged Attention | Spec Decoding | MLA | Sliding Window |
|---------|----------------|---------------|-----|----------------|
| FlashAttention (fa) | ✅ | ✅ | ❌ | ✅ |
| Native | ❌ | ❌ | ❌ | ❌ |
...
以下 models have been tested and optimized for TPU deployment:
...
1. 安装 SkyPilot and configure GCP access (see [SkyPilot documentation](https://skypilot.readthedocs.io/))
...
**NOTE:** Speculative decoding is currently supported for Qwen3 and LLaMA model families. See the [投机解码 documentation](https://github.com/sgl-project/sglang-jax/blob/main/docs/features/speculative_decoding.md) for detailed configuration guidance.


### Multi-Node Distributed 服务化
...
See the [基准测试与性能分析 Guide](https://github.com/sgl-project/sglang-jax/blob/main/docs/developer_guide/benchmark_and_profiling.md) for advanced benchmarking techniques and profiling with JAX Profiler.
...
See the [投机解码 documentation](https://github.com/sgl-project/sglang-jax/blob/main/docs/features/speculative_decoding.md) for detailed configuration and supported model combinations.
...
SGLang-JAX supports a plugin-based attention backend system. 你可以 implement custom attention kernels optimized for specific use cases.

See the [注意力后端 documentation](https://github.com/sgl-project/sglang-jax/blob/main/docs/features/attention_backend.md) for implementation details.
...
- 性能 optimizations for specific TPU generations
- 支持 for additional model architectures
- Documentation improvements and examples
- Bug reports and fixes
- 基准测试 results and performance analysis

### How to Contribute

1. Visit the [sglang-jax repository](https://github.com/sgl-project/sglang-jax)
2. Read the [贡献指南](https://github.com/sgl-project/sglang-jax/blob/main/docs/developer_guide/contribution_guide.md)
...
- Follow the [基准测试与性能分析 Guide](https://github.com/sgl-project/sglang-jax/blob/main/docs/developer_guide/benchmark_and_profiling.md) for performance validation
...
- [SGLang-JAX 安装 Guide](https://github.com/sgl-project/sglang-jax/blob/main/docs/get_started/install.md)
- [Qwen 模型 快速开始](https://github.com/sgl-project/sglang-jax/blob/main/docs/basic_usage/qwen.md)
- [基准测试与性能分析 Guide](https://github.com/sgl-project/sglang-jax/blob/main/docs/developer_guide/benchmark_and_profiling.md)
