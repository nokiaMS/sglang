...
本文档 describes how to run SGLang on AMD GPUs. If you encounter issues or have questions, please [open an issue](https://github.com/sgl-project/sglang/issues).
...
- [Supercharge DeepSeek-R1 推理 on AMD Instinct MI300X](https://rocm.blogs.amd.com/artificial-intelligence/DeepSeekR1-Part2/README.html)
...
你可以 automate or verify this change using [this helpful script](https://github.com/ROCm/triton/blob/rocm_env/scripts/amd/env_check.sh).
...
4. To verify the utility, you can run a benchmark in another terminal or refer to [other docs](/docs_CN/basic_usage/openai_api_completions.md) to send requests to the engine.
...
- `petit_nvfp4` enables NVFP4 models (e.g., [Llama 3.3 70B FP4](https://huggingface.co/nvidia/Llama-3.3-70B-Instruct-FP4)) on MI250/MI300X via [Petit](https://github.com/causalflow-ai/petit-kernel). 安装 with `pip install petit-kernel`; no `--quantization` flag needed when loading pre-quantized NVFP4 models.
