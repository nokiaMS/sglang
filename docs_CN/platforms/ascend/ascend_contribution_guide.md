...
Welcome to **SGLang**! We appreciate your interest in contributing. 本指南 provides a concise overview of how to set up your environment, run tests, build documentation, and open a Pull 请求 (PR). Whether you’re fixing a small bug or developing a major feature, we encourage following these steps for a smooth contribution process.

## 安装 SGLang from Source

### Prepare Environment

Before contributing, please ensure that your environment is set up correctly. Follow the steps in the [安装 Guide](ascend_npu.md) to install the necessary dependencies. We recommend [using docker](ascend_npu.md#method-2-using-docker-image) to build the environment.

### Fork and clone the repository

**注意**: New contributors do **not** have the write permission to push to the official SGLang repo. Please fork the repository under your GitHub account, then clone your fork locally.
...
- **`pre-commit run --all-files`** manually runs all configured checks, applying fixes if possible. If it fails the first time, re-run it to ensure lint errors are fully resolved. 请确保 your code passes all checks **before** creating a Pull 请求.
...
2. 请确保 your model is pre-cached on the CI server and is on the way "/data/ascend-ci-share-pkking-sglang/modelscope/hub/models/{your_model_repo}/{your_model}".
If this is not the case, use following command on CI server:
  ...
  ```
  > 注意: If you don’t have access to CI server, please ask maintainers (zl19940307@163.com) to download your model.
...
更多详细信息, please refer to [docs/README.md](https://github.com/sgl-project/sglang/tree/main/docs/README.md).
...
你可以 find additional accuracy eval examples in:
...
## 基准测试 the speed
Refer to [基准测试与性能分析](../../developer_guide/benchmark_and_profiling.md).

## Requesting a review for merge
你可以 follow the pull request merge process described in [MAINTAINER.md](https://github.com/sgl-project/sglang/blob/main/.github/MAINTAINER.md).
...
示例 of rerunning a single test stage: `/rerun-stage unit-test-backend-4-gpu`.
...
Each CI workflow has a default limit defined in its workflow configuration file. 例如, in [pr-gate.yml](https://github.com/sgl-project/sglang/blob/main/.github/workflows/pr-gate.yml), the default cooldown period is 120 minutes, and each workflow can override it via the `cool-down-minutes` input parameter:
