...
模型 weights could be found [here](https://huggingface.co/Qwen/Qwen3-32B)
...
模型 weights could be found [here](https://huggingface.co/Qwen/Qwen3-30B-A3B)
...
模型 weights could be found [here](https://huggingface.co/Qwen/Qwen3-235B-A22B-Instruct-2507)
...
> **注意:** `ASCEND_MF_STORE_URL` on both nodes must point to the same KV store (typically the Prefill node IP). `ASCEND_USE_FIA=True` enables fast interconnect aggregation for KV transfer. PCP is a Prefill-only feature; the Decode side needs no CP-related flags.

#### Running Qwen3-VL-8B-Instruct on 1 x Atlas 800I A3.

模型 weights could be found [here](https://huggingface.co/Qwen/Qwen3-VL-8B-Instruct)
