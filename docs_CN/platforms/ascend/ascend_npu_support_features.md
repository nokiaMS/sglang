...
如果你想要 know the meaning and usage of each parameter,
click [服务器参数](/docs_CN/advanced_features/server_arguments.md).

## 模型 and tokenizer

| 参数                               | Defaults | 选项                               | 服务器 supported |
|----------------------------------------|----------|---------------------------------------|:----------------:|
...
| `--pipeline-parallel-size`<br/>`--pp-size`               | `1`      | Type: int; 目前 `2` not supported |   Experimental   |
...
| 参数                                        | Defaults        | 选项                                                                       | 服务器 supported |
|-------------------------------------------------|-----------------|-------------------------------------------------------------------------------|:----------------:|
| `--enable-hierarchical-`<br/>`cache`            | `False`         | bool flag<br/> (set to enable).<br/> 目前, mamba cache is not supported. |      A2, A3      |
| `--hicache-ratio`                               | `2.0`           | Type: float                                                                   |      A2, A3      |
| `--hicache-size`                                | `0`             | Type: int                                                                     |      A2, A3      |
| `--hicache-write-policy`                        | `write_through` | 目前 only `write_back` supported                                         |      A2, A3      |
...
| `--enforce-piecewise-`<br/>`cuda-graph`                 | `False`  | bool flag<br/> (set to enable); <br/> 目前, Llama-3.1-8B-Instruct and Qwen2.5-7B-Instruct models are supported. |      A2, A3      |
...
以下 parameters are not supported because the third-party components that depend on are not compatible with the
...
以下 parameters have some functional deficiencies on community
