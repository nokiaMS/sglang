...
__Notice:__ 以下 commands are based on Atlas 800I A3 machines. If you are using Atlas 800I A2, some changes are needed.
...
The SGLang server is installed in the container by default. 你可以 use `pip show sglang` to check the version.
...
你可以 do inference using the server:
...
The SGLang server is running as a background process. 你可以 send a `SIGINT` signal to stop it.
...
The server has now stopped. 你可以 verify it with `ps -ef | grep sglang`, then exit the container by pressing `Ctrl+D`.
