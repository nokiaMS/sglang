...
你可以 install SGLang using any of the methods below. Please go through `System Settings` section to ensure the clusters are roaring at max performance. Feel free to leave an issue [here at sglang](https://github.com/sgl-project/sglang/issues) if you encounter any issues or have any problems.
...
### Obtain CANN Image
你可以 obtain the dependency of a specified version of CANN through an image.
```shell
...
如果你想要 use PD disaggregation mode, you need to install MemFabric-Hybrid. MemFabric-Hybrid is a drop-in replacement of Mooncake Transfer Engine that enables KV cache transfer on Ascend NPU clusters.
...
#### Obtain Image
你可以 download the SGLang image or build an image based on Dockerfile to obtain the Ascend NPU image.
...
__Notice:__ 以下 docker command is based on Atlas 800I A3 machines. If you are using Atlas 800I A2, make sure only `davinci[0-7]` are mapped into container.
...
### Running Service For Large Language 模型
#### PD Mixed Scene
```shell
