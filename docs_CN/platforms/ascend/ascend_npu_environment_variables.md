...
SGLang 支持 various environment variables related to Ascend NPU that can be used to configure its runtime behavior.
本文档 provides a list of commonly used environment variables and aims to stay updated over time.
...
| `SGLANG_USE_FIA_NZ`                              | Reshapes KV 缓存 for FIA NZ format.<br/> `SGLANG_USE_FIA_NZ` must be enabled with `SGLANG_NPU_USE_MLAPO`                                                   | `false`       |
