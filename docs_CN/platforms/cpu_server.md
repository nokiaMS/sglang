...
**注意:** The model identifiers listed in the table above
...
建议 to use Docker for setting up the SGLang environment.
...
你可以 refer to the Ubuntu-based installation commands in
[the Dockerfile](https://github.com/sgl-project/sglang/blob/main/docker/xeon.Dockerfile#L11)
for guidance.

1. 安装 `uv` package manager, then create and activate a virtual environment:
...
- 注意 that the environment variable `SGLANG_USE_CPU_ENGINE=1`
...
- If you encounter code compilation issues during the `sgl-kernel` building process,
    ...
    建议 to use `gcc-13` and `g++-13` as they have been verified
...
- 建议 to add the following to your `~/.bashrc` file to
...
示例 command to launch SGLang serving:
...
2. The flag `--tp 6` specifies that tensor parallelism will be applied using 6 ranks (TP6).
    ...
    注意 that `n` cannot exceed the total SNC number, doing so will result in an error.
...
3. For optimizing decoding with torch.compile, please add the flag `--enable-torch-compile`.
    ...
    例如, `--enable-torch-compile --torch-compile-max-bs 4` means using `torch.compile`
...
你可以 benchmark the performance via the `bench_serving` script.
...
[the OpenAI Completions API](/docs_CN/basic_usage/openai_api_completions.md)
and sent via the command line (e.g., using `curl`) or through your own scripts.

## 示例 用法 Commands

Large Language 模型 can range from fewer than 1 billion to several hundred billion parameters.
...
### 示例: Running DeepSeek-V3.1-Terminus
...
注意: Please set `--torch-compile-max-bs` to the maximum desired batch size for your deployment,
which can be up to 16. The value `4` in the examples is illustrative.

### 示例: Running Llama-3.2-3B
...
注意: The `--torch-compile-max-bs` and `--tp` settings are examples that should be adjusted for your setup.
