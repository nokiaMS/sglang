...
### 配置 Tips
- **DP Attention**: 要启用 [DP Attention](../advanced_features/dp_dpa_smg_guide.md), please include `--enable-dp-attention --dp <dp-size>` in command. DP Attention is better for large concurrency scenarios.
...
- **Index 缓存**: Introduce in [this paper](https://arxiv.org/abs/2603.12201), IndexCache improves speed by reusing the result of indexer across different layers, only at cost of negligible accuracy loss.  For **GLM-5** model, we recommend appending `--json-model-override-args '{"index_topk_pattern": "FFSFSSSFSSFFFSSSFFFSFSSSSSSFFSFFSFFSSFFFFFFSFFFFFSFFSSSSSSFSFFFSFSSSFSFFSFFSSS"}'` to command for better tradeoff between speedup and performance.

## Multi-token Prediction
SGLang implements Multi-Token Prediction (MTP) for DeepSeek V3.2 based on [EAGLE speculative decoding](/docs_CN/advanced_features/speculative_decoding.md#EAGLE-Decoding). With this optimization, the decoding speed can be improved significantly on small batch sizes. Please look at [this PR](https://github.com/sgl-project/sglang/pull/11652) for more information.

示例 usage with DP Attention:
```bash
...
The usage of function calling and reasoning parser is the same as DeepSeek V3.1. 请参考 [Reasoning Parser](/docs_CN/advanced_features/separate_reasoning.md) and [Tool Parser](/docs_CN/advanced_features/tool_parser.md) documents.

To launch `DeepSeek-V3.2-Exp` with function calling and reasoning parser:
> 注意: 建议 to specify the chat-template, ensuring that you are within the sglang's root directory.
```bash
...
**注意: This feature is only verified on Hopper machines**

For context parallel in DeepSeek V3.2 model, we provide two different modes of splitting tokens, which can be controlled with argument `--dsa-prefill-cp-mode` (the deprecated `--nsa-prefill-cp-mode` alias also works).
...
注意 that the in-sequence splitting mode has the following restrictions:
...
更多详细信息, please refer to PR https://github.com/sgl-project/sglang/pull/12065.
...
更多详细信息, please refer to PR https://github.com/sgl-project/sglang/pull/13959.
...
This mode combines 流水线并行 (PP) and Context Parallelism (CP) to scale across multiple nodes, which can achieve better throughput and Time To First Token (TTFT). 注意 that this method has only been tested on H20 96G.
