# GLM-4.7-Flash MoE Lite 推测解码(NextN)模型文件
# 本文件实现了仅推理模式的 GLM-4.7-Flash MoE Lite 推测解码(NextN)模型，
# 兼容 HuggingFace 权重格式，用于加速推理过程中的草稿 token 生成。

# Copyright 2026-2027 SGLang Team # 版权所有 2026-2027 SGLang 团队
...
"""Inference-only GLM-4.7-Flash Speculative Decoding (NextN) compatible with HuggingFace weights."""  # 仅推理的 GLM-4.7-Flash 推测解码(NextN)模型，兼容 HuggingFace 权重
...
from sglang.srt.models.glm4_moe_lite import (  # 从 glm4_moe_lite 模型导入
    Glm4MoeLiteDecoderLayer,  # GLM4 MoE Lite 解码器层
    Glm4MoeLiteForCausalLM,  # GLM4 MoE Lite 因果语言模型
)
from sglang.srt.server_args import get_global_server_args  # 导入获取全局服务器参数的函数
from sglang.srt.utils import BumpAllocator, add_prefix, is_npu  # 导入缓冲分配器、前缀添加和 NPU 判断工具

logger = logging.getLogger(__name__)  # 获取当前模块的日志记录器


class Glm4MoeLiteModelNextN(nn.Module):  # GLM-4.7-Flash MoE Lite NextN 模型类
    def __init__(  # 初始化方法
        self,
        ...
        if quant_config is not None and quant_config.get_name() == "modelopt_fp4":  # 如果量化配置为 modelopt_fp4
            logger.warning(  # 记录警告
                "Overriding Glm4MoeLiteForCausalLMNextN quant config for modelopt_fp4 "  # 覆盖 modelopt_fp4 的量化配置
                "GLM-4.7-Flash model."  # GLM-4.7-Flash 模型
            )
        ...
        self.decoder = Glm4MoeLiteDecoderLayer(  # 创建 MoE Lite 解码器层
            config,  # 模型配置
            0,  # 层索引为0
    ...
    def forward(  # 前向传播方法
        ...
        # Glm4MoeLiteDecoderLayer uses DeepseekV2AttentionMLA, which requires a  # Glm4MoeLiteDecoderLayer 使用 DeepseekV2AttentionMLA，需要一个
        # zero_allocator (the GQA glm4_moe_nextn path does not pass one).  # zero_allocator（GQA glm4_moe_nextn 路径不传递此参数）
        zero_allocator = BumpAllocator(  # 创建缓冲分配器
            buffer_size=2,  # 缓冲区大小为2
            dtype=torch.float32,  # 数据类型为 float32
            device=(  # 设备
                input_embeds.device if input_embeds is not None else input_ids.device  # 使用输入嵌入或输入ID的设备
            ),
        )
        ...
        with get_global_expert_distribution_recorder().disable_this_region():  # 在禁用专家分布记录的区域内执行
            hidden_states, residual = self.decoder(  # 通过解码器层
                positions, hidden_states, forward_batch, residual, zero_allocator  # 传入位置、隐藏状态、批次信息、残差和分配器
            )
...
class Glm4MoeLiteForCausalLMNextN(Glm4MoeLiteForCausalLM):  # GLM-4.7-Flash MoE Lite 因果语言模型 NextN 类，继承自 Glm4MoeLiteForCausalLM
    def __init__(  # 初始化方法
        self,
        config: PretrainedConfig,  # 预训练配置
        quant_config: Optional[QuantizationConfig] = None,  # 量化配置，可选
        ...
        self.model = Glm4MoeLiteModelNextN(  # 创建 NextN 模型
...
EntryClass = [Glm4MoeLiteForCausalLMNextN]  # 入口类列表，用于模型注册
